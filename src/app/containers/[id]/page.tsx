...
import { ArrowLeft, Play, Square, RotateCcw, Trash2, Terminal, Pause } from "lucide-react";
import docker from "@/lib/docker";
import { notFound } from "next/navigation";
import { ContainerActions } from "./container-actions";

export default async function ContainerDetailsPage(props: { params: Promise<{ id: string }> }) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white/40 dark:bg-zinc-950/40 backdrop-blur-xl p-5 rounded-2xl border border-white/20 dark:border-zinc-800/50 shadow-lg">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <span className="font-mono text-sm truncate block" title={details.Id}>{details.Id.substring(0, 12)}</span>
                            </div>
                            <div>
                                <span className="text-sm text-zinc-500 block">Image</span>
                                <span className="font-mono text-sm truncate block" title={details.Config.Image}>{details.Config.Image}</span>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <span className="font-mono text-sm truncate block">{Array.isArray(details.Config.Entrypoint) ? details.Config.Entrypoint.join(' ') : details.Config.Entrypoint || '-'}</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="bg-white/40 dark:bg-zinc-950/40 backdrop-blur-xl p-5 rounded-2xl border border-white/20 dark:border-zinc-800/50 shadow-lg">
                            ...
                            <div className="space-y-3">
                                {Object.entries(details.NetworkSettings.Networks).map(([netName, netData]: [string, any]) => (
                                    <div key={netName} className="bg-zinc-100/50 dark:bg-zinc-900/50 rounded-lg p-3 text-sm">
                                        <div className="font-medium text-zinc-900 dark:text-zinc-100 mb-2">{netName}</div>
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
                                            <div>
                                                <span className="text-zinc-500 block mb-0.5">IP-Adresse</span>
                                                <span className="font-mono text-zinc-700 dark:text-zinc-300">{netData.IPAddress || '-'}</span>
                                            </div>
                                            <div>
                                                <span className="text-zinc-500 block mb-0.5">Gateway</span>
                                                <span className="font-mono text-zinc-700 dark:text-zinc-300">{netData.Gateway || '-'}</span>
                                            </div>
                                            <div>
                                                <span className="text-zinc-500 block mb-0.5">Mac Address</span>
                ...
                <div className="space-y-6">
                    <div className="bg-white/40 dark:bg-zinc-950/40 backdrop-blur-xl p-5 rounded-2xl border border-white/20 dark:border-zinc-800/50 shadow-lg flex flex-col gap-2">
                        ...
                        <Link href={`/containers/${id}/terminal`} className={!isRunning ? 'pointer-events-none opacity-50' : ''} passHref>
                    ...
                    <div className="bg-white/40 dark:bg-zinc-950/40 backdrop-blur-xl p-5 rounded-2xl border border-white/20 dark:border-zinc-800/50 shadow-lg flex flex-col gap-2">
                        ...
                        <ContainerActions id={id} isRunning={isRunning} isPaused={isPaused} />
                    </div>

                    {details.Mounts && details.Mounts.length > 0 && (
                        <div className="bg-white/40 dark:bg-zinc-950/40 backdrop-blur-xl p-5 rounded-2xl border border-white/20 dark:border-zinc-800/50 shadow-lg">
                            <h3 className="text-lg font-medium mb-4">Mounts ({details.Mounts.length})</h3>
                            <ul className="space-y-3">
                                {details.Mounts.map((mount: any, idx: number) => (
                                    <li key={idx} className="bg-zinc-100/50 dark:bg-zinc-900/50 p-2.5 rounded-lg text-xs break-all">
                                        <div className="flex gap-2 mb-2">
                                            <Badge variant="secondary" className="text-[10px] px-1.5 py-0 h-4">{mount.Type}</Badge>
                                            <Badge variant="outline" className="text-[10px] px-1.5 py-0 h-4">{mount.RW ? 'RW' : 'RO'}</Badge>
                                        </div>
                                        <div className="flex flex-col gap-1 font-mono text-[10px] text-zinc-600 dark:text-zinc-400 bg-white/50 dark:bg-black/20 p-2 rounded border border-zinc-200/50 dark:border-zinc-800/50">
                                            ...
                                            <span className="text-center text-zinc-400 opacity-50">↓</span>
                                            <span className="text-zinc-800 dark:text-zinc-200">{mount.Destination}</span>
                                        </div>
                                    </li>
                                ))}
                            </ul>
                    ...
                    <div className="bg-zinc-100/50 dark:bg-zinc-900/50 rounded-lg p-4 font-mono text-sm max-h-96 overflow-y-auto">
                        <ul className="space-y-1.5 break-all">
                            {details.Config.Env.map((env: string, idx: number) => {
                                ...
                                return (
                                    <li key={idx} className="flex flex-col sm:flex-row gap-1 sm:gap-6">
                                        <span className="text-blue-600 dark:text-blue-400 font-semibold min-w-[200px] shrink-0">{key}</span>
                                        <span className="text-zinc-600 mt-0.5 sm:mt-0 dark:text-zinc-400">{val || '""'}</span>
                                    </li>
                                );
                            })}
                        </ul>
