    ...
    const stacks = await Promise.all(
      dirs
        ...
        .map(async (dirent) => {
          ...
          let composeFile = "";
          try {
            composeFile = await fs.readFile(
              path.join(stackPath, "docker-compose.yml"),
              "utf-8",
            );
          } catch (e) {
            // Try .yaml extension
            try {
              composeFile = await fs.readFile(
                path.join(stackPath, "docker-compose.yaml"),
                "utf-8",
              );
            } catch (e2) {
              // Ignore
  ...
  try {
    ...
    if (!name || !composeFile) {
      return NextResponse.json(
        { error: "Name and compose file content are required" },
        { status: 400 },
      );
    }
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
      return NextResponse.json(
        {
          error:
            "Invalid stack name. Only alphanumeric, dashes and underscores allowed.",
        },
        { status: 400 },
      );
    }
