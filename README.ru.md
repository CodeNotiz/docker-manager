...
(больше изображений в [docs](docs/))
...
|---|---|---|
| `LOG_LEVEL` | `INFO` | Определяет уровень детализации серверных логов. |
| `PORT` | `3000` | Порт, на котором слушает Node.js-сервер. |
| `HOST` | `0.0.0.0` | Сетевой интерфейс, к которому привязывается сервер. |
| `COOKIE_SECURE` | `false` | Установить в `true` только если сервер работает за HTTPS/обратным прокси. |

### Уровни логирования (`LOG_LEVEL`)

| Значение | Описание |
|---|---|
| `DEBUG` | Все сообщения, включая socket-подключения и middleware-редиректы |
| `INFO` | Стандарт – запуск сервера, инициализация БД, ошибки *(по умолчанию)* |
| `WARN` | Только предупреждения и ошибки (напр. истёкшие токены) |
| `ERROR` | Только ошибки |
| `SILENT` | Без вывода |
